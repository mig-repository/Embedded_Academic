...
unsigned char vect_disp[4] = {10,0,11,11};			// Para o display mostrar OFF no in�cio
...
void inicio(void)									//Configura��o dos PORTS e do USART1
{
	DDRA=0b11000000;								// Switchs como entradas e multiplexer dos displays como saida
...
void display(void)												//Rotina que ir� expor um display diferente a cada 5ms.
{
...
void processar_display(void)									//Exibi��o dos display.
{
...
ISR(TIMER0_COMP_vect)											//Interrupcao do timer.
{
	...
	if (flag_interrupt == 1)									// Flag ativa se houver um pedido de invers�o de sentido do motor.
	{
...
ISR (USART1_RX_vect)											//Interrup��o da comunica��o serie - USART.
{
	rxUSART.status = UCSR1A;									// Guarda flags de erros
	
	if (rxUSART.status & ((1<<FE1)|(1<<DOR1)|(1<<UPE1))) 		// Verifica erros na rece��o
	{
...
int main(void)
{
    ...
    while (1) 
    {
		ler_switch();											// Fica � espera de ler qual o butao premido.
	
		if (rxUSART.receive == 1)								// Verifica se recebeu algum dado.
		{
			if (rxUSART.error == 1)								// Verifica se existe algum erro.
			{
																// Procedimentos para resolver erros.
				rxUSART.error = 0; 								// Limpa flag de erro na rece��o
			}

			ler_dado_recebido();								// Executa a��es consoante o valor recebido.

			rxUSART.receive = 0; 								// Limpa flag de aviso de rece��o de dados.
		}
    }
}
